...
def load_frame_processor_module(frame_processor : str) -> Any:
	try:
		frame_processor_module = importlib.import_module('facefusion.processors.frame.modules2.' + frame_processor) #importlib.import_module动态导入模块
		for method_name in FRAME_PROCESSORS_METHODS:
			if not hasattr(frame_processor_module, method_name):   #hasattr判断对象是否包含对应的属性
...
def clear_frame_processors_modules() -> None:  #清理处理模型，不是模块
	global FRAME_PROCESSORS_MODULES
