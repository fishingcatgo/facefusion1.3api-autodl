...
if __name__ == '__main__':
    ...
    for face in facelist:
        ...
        if os.path.isdir(f'{path_imgs}/int/{nameface}'):
            imglist = os.listdir(f'{path_imgs}/int/{nameface}')
        else:
            print('要换脸图片路径不存在：',f'{path_imgs}/int/{nameface}')
            continue
        
        dir_name=path_imgs+'out/'+nameface
        if not os.path.isdir(dir_name):
            os.makedirs(dir_name)
    
        print('imglist图片:',imglist)
        for value in imglist:
            # print('值：',path_imgs+value)
            # data['source_path']=path_imgs+'face/717.jpg'
            # data['target_path']=path_imgs+'int/'+value
            data['target_path']=f'{path_imgs}/int/{nameface}/{value}'
